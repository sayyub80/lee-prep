...
export default function GroupChatPage() {
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    setMessages(data.data.messages.map((msg: any) => ({ 
                        _id: msg._id,
                        sender: { id: msg.sender.toString(), name: msg.senderName, avatar: msg.sender.avatar }, 
                        text: msg.text 
                    })));
                }
        ...
        socket.on('connect', () => {
            socket.emit('join-group', { groupId, user: { id: user.id, name: user.name, avatar: user.avatar } });
        });
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        if (newMessage.trim() && user && socketRef.current) {
            const messageData = { groupId, sender: { id: user.id, name: user.name, avatar: user.avatar }, text: newMessage };
            ...
            <div className="flex h-full border rounded-xl shadow-lg bg-card overflow-hidden">
                ...
                <div className="flex flex-col h-full flex-1">
                    <header className="flex items-center p-4 border-b">
                        <Button variant="ghost" size="icon" className="mr-2" onClick={() => router.push('/practice/groups')}><ArrowLeft /></Button>
                        <div>
                    ...
                    <main ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-6">
                        ...
                        {group?.currentTopic?.title && (
                            <div className="bg-background border-l-4 border-primary p-4 rounded-r-lg mb-2 self-center w-full max-w-2xl shadow-sm mx-auto">
                                <div className="flex items-start gap-3">
                                    <Info className="h-5 w-5 text-primary mt-1 flex-shrink-0" />
                                    <div>
                                        <p className="font-semibold text-sm">Today's Topic</p>
                                        <p className="font-medium">{group.currentTopic.title}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <footer className="p-4 border-t bg-background">
                        <form className="flex gap-2" onSubmit={handleSendMessage}>
                            ...
                            <Input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..." autoComplete="off" className="h-10"/>
                            <Button type="submit" size="icon" className="h-10 w-10"><Send/></Button>
                        </form>
                    </footer>
                </div>
                ...
                <aside className="w-80 border-l flex-col hidden lg:flex bg-secondary/30">
                    ...
                    <div className="p-6 flex-1 overflow-y-auto">
                        <h3 className="font-semibold mb-4 flex items-center">
                            <span className="relative flex h-3 w-3 mr-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span></span>
                            ...
                            {onlineUsers.map(onlineUser => (
                                <div key={onlineUser.id} className="flex items-center gap-3">
                                    <Avatar className="h-9 w-9">
                                        ...
                                        <AvatarFallback>{onlineUser.name?.trim().charAt(0).toUpperCase() || '?'}</AvatarFallback>
                                    </Avatar>
